...
def get_white_pixel_percentage(mask):
    """
    Berechnet den Prozentsatz der weißen Pixel in einer Maske.

    Args:
    - mask (PIL Image): Maskenbild.

    Returns:
    - float: Prozentsatz der weißen Pixel.
    """
    mask_array = np.array(mask)
    
    # Zähle weiße Pixel
    white_pixel_count = get_white_pixel_count(mask)
    
    # Berechne die Gesamtzahl der Pixel
    total_pixel_count = mask_array.size  # Gesamtzahl der Pixel im Bild
    
    # Berechne den Prozentsatz der weißen Pixel
...
def save_mask_sizes_to_csv(masks_dir, output_csv):
    ...
    with open(output_csv, mode='w', newline='') as file:
        ...
        writer.writerow(["Bildname", "Maskengröße (weiße Pixel)", "white_pixel_percentage"])
        
        for fname, mask in mask_generator(masks_dir, mask_files):
            mask_size = get_white_pixel_count(mask)
            white_pixel_percentage = get_white_pixel_percentage(mask)
            writer.writerow([fname, mask_size, white_pixel_percentage])
